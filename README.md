# Code used for the decoding analysis and data loading/analysis/plotting.

This file provides a brief summary of the Python and MATLAB scripts in this directory.
...
in .mat or .npy.gz files! See Zenodo repository for this manuscript).
...
This script reads and processes pickle files containing results from the `dropping_accuracy_filter_trials.py` analysis. It aggregates data from multiple simulation runs and plots the mean accuracy as a function of feature removal.

### `run_bl_analysis.py`
This script evaluates the performance of a classifier on background spiking activity. It compares the decoding accuracy of a model trained on spontaneous activity to one trained on stimulus-evoked activity.
...
This script analyzes how classifier accuracy changes over time relative to a perturbation stimulus. It runs a decoding analysis in a sliding window around the stimulus onset and saves the time-resolved performance.

### `plot_vstime_pertstim.py`
This script plots the accuracy of a classifier over time relative to a perturbation stimulus. It reads analysis results from pickle files and visualizes how decoding performance changes around the stimulus event.
